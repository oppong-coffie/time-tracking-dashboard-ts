const buttons: NodeListOf<Element> = document.querySelectorAll('.activity-tracker__option');
let data: any[] = [];

const listenToButtons = (array: Element[]) => {
...
const activateClickedButton = (button: Element) => {
...
const renderCards = async (clickedOption: string) => {
    clearActivities();
    const activityTracker = document.querySelector('section.activity-tracker');

    const calcTimeframe = (option: string): string => {
        if (option === 'daily') {
            return 'Yesterday';
